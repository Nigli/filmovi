...
foreach ($bookmarks as $row) {
    ...
    if (property_exists($row, 'imdb_id')) {
        ...
        $trailer_id = getBetween(file_get_contents("http://gdata.youtube.com/feeds/api/videos?q=$movie_name_string-trailer&max-results=1&alt=json",
            null, null), '?v=', '&feature');
        Listafilmova::InsertMovie($row->imdb_id, $omdb->Title, $trailer_id, $omdb->Year, $omdb->Runtime,
            $omdb->Director, $omdb->Actors, $omdb->Poster);
    }
}
