...
<form class="user_insert" action="../user_insert.php" method="post">
    ...
    <input class="user_submit" type="submit" name="submit" onClick="return confirm('Uneti novog korisnika?')"
           value="unesi">
</form>
<h3>Ovo je trenutna lista korisnika</h3>
<?php
foreach (Users::GetAllFromUsers() as $k => $user) {
    ?>
    <form class="raw_user" action="../user_delete.php" method="post">
        ...
        <input class="user_submit" type="submit" name="delete"
               onClick="return confirm('Obrisati korisnika <?php echo $user->name ?>?');" value="obriši"/>
    </form>
    <?php
}
